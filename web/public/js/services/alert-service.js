...
angular.module('cpZenPlatform').factory('alertService',['$rootScope', '$translate', function($rootScope, $translate) {
  ...
  alertService.debugError = function(bugName, bugDump, callback) {
    ...
    bootbox.dialog({
      ...
      message: '<span class="text-danger">'+ $translate.instant('We\'re trying to track down what produces this bug :') + bugName +'</span>'+
              '<div>'+ $translate.instant('Please use the opened intercom messenger to report more details about it!') + '</div>'+
